...
def GetFileSize(filePath):
    ...
    try:
        ...
        mbSize = round(bytesSize / (1024 * 1024), 3)
...
def WriteConfigFile(requestFormItems, section):
    try:
        config = configparser.ConfigParser()
        config.read(globalConstants.ConfigurationFile)

        if (section == globalConstants.APIConfigurationSection):
            if globalConstants.APIConfigurationSection not in config:
                config[globalConstants.APIConfigurationSection] = {}

            for key, value in requestFormItems:
                config[globalConstants.APIConfigurationSection][key] = value

        elif (section ==globalConstants.Pay2UseAnalyzersConfigurationSection):
            if globalConstants.Pay2UseAnalyzersConfigurationSection not in config:
                config[globalConstants.Pay2UseAnalyzersConfigurationSection] = {}

            received_keys = set()

            # Save received checkbox values as 'on' or 'off'
            for key, value in requestFormItems:
                config[globalConstants.Pay2UseAnalyzersConfigurationSection][key] = 'on' if value == 'on' else 'off'
                received_keys.add(key)

            # Set default 'off' for checkboxes not present in the request
            if globalConstants.expectedCheckboxesPay2UseAnalyzers:
                for key in globalConstants.expectedCheckboxesPay2UseAnalyzers:
                    if key not in received_keys:
                        config[globalConstants.Pay2UseAnalyzersConfigurationSection][key] = 'off'

        with open(globalConstants.ConfigurationFile, 'w') as configfile:
            config.write(configfile)
        return True
    except Exception as e:
        print(f"[utils.WriteConfigFile] - An error occurred: {e}")
        return False
...
def normalize_path(path: str) -> str:
    """Normalizza un percorso in base al sistema operativo."""
    return str(Path(path).resolve())


def map_message_types(messages):
    """
    Converte una lista di stringhe nei rispettivi interi basati su un mapping specifico.

    :param messages: Lista di stringhe rappresentanti i tipi di messaggi.
    :return: Lista di interi corrispondenti ai tipi di messaggio.
    """
    ...
    # Funzione di supporto per mappare un messaggio
    def map_message(message):
        if message == 'text':
            return mapping['text']
        elif message == 'image':
            return 1  # Può essere qualsiasi valore nell'intervallo 1-42
        elif message == 'audio':
            return mapping['audio']
        elif message == 'video':
            return 3  # Può essere qualsiasi valore nell'intervallo 3-43
        elif message == 'gif':
            return mapping['gif']
        elif message == 'location':
            return mapping['location']
        elif message == 'groupEvent':
            return mapping['groupEvent']
        elif message == 'url':
            return mapping['url']
        elif message == 'file':
            return mapping['file']
        else:
            return -1  # Default per messaggi non riconosciuti

    # Applicazione del mapping a ciascun messaggio nella lista
    return [map_message(msg) for msg in messages]