...
def GetChatList(basePath, deviceUdid):
    ...
    databasePath = basePath + "/" + globalConstants.deviceExtractions_FOLDER_IOS + "/" + deviceUdid + globalConstants.defaultDatabase_PATH
...
def GetMediaFromBackup(basePath, deviceUdid, filePath, isChatMedia, isProfilePic):
    ...
    manifestDBFilePath = basePath + "/" + os.path.join(globalConstants.deviceExtractions_FOLDER_IOS, deviceUdid, globalConstants.manifestDBFile)
    ...
    with sqlite3.connect(manifestDBFilePath) as manifest:
        ...
        while row is not None:
            ...
            flags = row["flags"]
            if flags == 1:
                imagePath = os.path.join(globalConstants.deviceExtractions_FOLDER_IOS, deviceUdid, folder, hashes)
