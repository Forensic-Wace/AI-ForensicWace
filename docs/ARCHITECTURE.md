...
| **7. Analyzer marketplace** 🚧 | Runtime-installable analyzers: fw-analyzer/1 HTTP contract, `analyzers` registry table, signed catalog, marketplace UI, on-demand container provisioning (scale-to-zero) | Phase A ✅ (contract + dynamic registry + BYO registration + finding provenance — see [analyzer-contract.md](analyzer-contract.md)); phases B (catalog + UI) and C (orchestration) tracked in [ANALYZER_MARKETPLACE.md](ANALYZER_MARKETPLACE.md) |
