...
| **6. Projects & object storage** ✅ | Project (case) entity + browser ZIP upload of backups; S3/MinIO evidence store (`forensicwace_core.storage`, boto3 behind the `s3` extra, `file://` backend for dev); hydration into the extraction roots; MinIO in compose + Helm | Backups uploaded from the UI land in the bucket (`projects/<id>/<platform>/<identifier>/...`), hydrate as working copies and are usable by every existing endpoint; zip-slip-safe extraction; feature degrades to 503 when unconfigured |
| **7. Analyzer marketplace** ✅ | Runtime-installable analyzers: fw-analyzer/1 HTTP contract, `analyzers` registry table, signed catalog, marketplace UI, on-demand container provisioning (scale-to-zero) | Phase A (contract + dynamic registry + BYO registration + finding provenance — see [analyzer-contract.md](analyzer-contract.md)); phase B (signed catalog + marketplace page + cloud consent); phase C (k8s/docker provisioner, idle scale-to-zero, GPU) — details in [ANALYZER_MARKETPLACE.md](ANALYZER_MARKETPLACE.md) |
